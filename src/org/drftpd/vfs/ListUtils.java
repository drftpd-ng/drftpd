...
public class ListUtils {
    ...
    public static List<InodeHandleInterface> list(DirectoryHandle directoryFile,
        BaseFtpConnection conn) {
        try {
			return list(directoryFile, conn, null);
		} catch (IOException e) {
			logger.info("IOException while listing directory "+directoryFile.getPath(),e);
			return new ArrayList<InodeHandleInterface>();
		}
    }

    public static List<InodeHandleInterface> list(DirectoryHandle dir,
        ...
        ArrayList<InodeHandleInterface> listFiles = new ArrayList<InodeHandleInterface>();
        int numOnline = 0;
        int numTotal = 0;
        //boolean id3found = false;
        //ID3Tag mp3tag = null;

        for (Iterator<InodeHandle> iter = tempFileList.iterator(); iter.hasNext();) {
            InodeHandle element = iter.next();

            if ((GlobalContext.getGlobalContext().getConfig() != null) &&
                    !GlobalContext.getGlobalContext().getConfig().checkPathPermission("privpath", conn.getUserNull(), dir, true)) {
                // don't add it
                continue;
            }
...
/*            if (element.isFile() &&
            ...
            //				continue;
            //			} else if (
            if (conn.getGlobalContext().getZsConfig().offlineFilesEnabled() && element.isFile()) {
