...
/*
 * @author iamn
 * @version $Id: MoreStats.java,v 1.1 2004/09/12 01:11:16 zubov Exp $
 */
...
public class MoreStats implements CommandHandlerFactory, CommandHandler {
	...
	public class MyGroupPosition extends GroupPosition {
		int members;
		int racesWon;
		public MyGroupPosition(String groupname,long bytes,int files,long xfertime,int members, int racesWon) {
    ...
    private FtpReply doSITE_GROUPSTATS(BaseFtpConnection conn) {
		...
		try {
			Textoutput.addTextToResponse(response, "g" + type.toLowerCase() + "_header");
		} catch (IOException ioe) {
			logger.warn("Error reading " + "g" + type.toLowerCase() + "_header");
		}
		int i = 0;
		for (Iterator iter = grpList.iterator(); iter.hasNext();) {
			...
			response.addComment(ReplacerUtils.jprintf("morestats.grpstats",env,MoreStats.class));

		}
		try {
			Textoutput.addTextToResponse(response, "g" + type.toLowerCase() + "_footer");
		} catch (IOException ioe) {
			logger.warn("Error reading " + "g" + type.toLowerCase() + "_footer");
		}
		return response;
	}
    ...
    private void AddTrafficComment(String type,double avrage, long megs, int files, FtpReply response) {
		double s = avrage / (double) 1000.0;
		if (s <= 0) { avrage = 0;}
		else {avrage = megs / s;}
		...
		response.addComment(ReplacerUtils.jprintf("morestats.trafficstats",env,MoreStats.class));
    }
