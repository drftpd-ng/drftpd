...
/**
 * @author mog
 * @version $Id: SocketSlaveImpl.java,v 1.6 2004/08/03 20:14:09 zubov Exp $
 */
public class SocketSlaveImpl extends Thread implements Slave, Unreferenced {
    private static final Logger logger = Logger.getLogger(SocketSlaveImpl.class);
    ...
    private void processRename(Command cmd) {
        String msg = "ren " + "\"" + cmd.getArgs().get("from") + "\" " + "\"" +
    ...
    private void processSend(Command cmd) {
        String msg = "send" + " \"" + cmd.getArgs().get("path") + "\"" + " " +
            cmd.getArgs().get("conn") + " " + cmd.getArgs().get("offs");
        sendLine(msg);
    ...
    private void processRecv(Command cmd) {
        String msg = "recv" + " \"" + cmd.getArgs().get("path") + "\"" + " " +
            cmd.getArgs().get("conn") + " " + cmd.getArgs().get("offs");
        sendLine(msg);
