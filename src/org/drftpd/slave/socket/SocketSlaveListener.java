...
public class SocketSlaveListener extends Thread {
    ...
    static void invalidSlave(String msg, Socket sock) throws IOException {
        ...
        try {
            ...
            String hash = SocketSlaveListener.hash2hex(md5.digest())
                                             .toLowerCase();
        ...
        while (true) {
            ...
            for (Iterator i = slaves.iterator(); i.hasNext();) {
                ...
                if (rslave.isAvailable()) {
                    logger.info("SockSlaveListener: online> " +
                        rslave.getName());
                ...
                if (saddr == null) {
                    logger.info("SockSlaveListener: noaddr> " +
                        rslave.getName());
                ...
                if (!saddr.equals("Dynamic")) {
                    logger.info("SockSlaveListener: static> " +
                        rslave.getName());
                ...
                for (Iterator i2 = rslave.getMasks().iterator(); i2.hasNext();) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("Invalid glob pattern: " +
                            mask, ex);
                    }
            ...
            if (match) {
                ...
                try {
                    SocketSlaveImpl tmp = new SocketSlaveImpl(_conman,
                            thisone.getConfig(), slave);
                } catch (Exception e) {
                }
            } else {
                // allow the slave to auto-register
                try {
                    SocketSlaveListener.invalidSlave("INITFAIL Unregistered",
                        slave);
                } catch (Exception e) {
                }
            }
