...
/**
 * @author mog
 * @version $Id: AsyncSlave.java,v 1.7 2004/08/03 20:14:08 zubov Exp $
 */
public class AsyncSlave extends Thread implements Slave, Unreferenced {
    private static final Logger logger = Logger.getLogger(AsyncSlave.class);
    ...
    public void init(ConnectionManager mgr, Hashtable cfg, Socket sock)
        ...
        try {
            ...
            _sinp = new BufferedReader(new InputStreamReader(
            ...
            _cman.getGlobalContext().getSlaveManager().addSlave(_name, this,
                getSlaveStatus(), -1);
            start();
        } catch (IOException e) {
            if (e instanceof ConnectIOException &&
    ...
    public void run() {
        ...
        while (true) {
            ...
            while (msg != "") {
                ...
                if (cmd._name.equals("ping")) {
                    processPing(cmd, msg);
                } else if (cmd._name.equals("xfer")) {
                    xferMessage(msg);
                } else if (cmd._name.equals("disk")) {
                    processDisk(cmd, msg);
                } else if (cmd._name.equals("conn")) {
                    processConnect(cmd, msg);
                } else if (cmd._name.equals("send")) {
                    processSend(cmd, msg);
                } else if (cmd._name.equals("recv")) {
                    processRecv(cmd, msg);
                } else if (cmd._name.equals("renm")) {
                    processRename(cmd, msg);
                } else if (cmd._name.equals("dele")) {
                    processDelete(cmd, msg);
                } else if (cmd._name.equals("list")) {
                    processList(cmd, msg);
                } else if (cmd._name.equals("csum")) {
                    processCRC32(cmd, msg);
                } else if (cmd._name.equals("dump")) {
                    processDump(cmd, msg);
                }
    ...
    private void shutdown() {
        ...
        try {
            _cman.getGlobalContext().getSlaveManager().delSlave(_name,
                "Connection lost");
        } catch (Exception e) {
        }
    }
    ...
    private void processList(AsyncCommand cmd, String res) {
        ...
        if (res.startsWith("LISTFAIL")) {
            ...
            if (res.equals("LISTEND")) {
                try {
                    LinkedRemoteFile root = MLSTSerialize.unserialize(_cman.getGlobalContext()
                                                                           .getConfig(),
                            new StringReader(buf.toString()),
                            _cman.getGlobalContext().getSlaveManager()
                                 .getSlaves());
                    _root = root;
                } catch (Exception e) {
    ...
    public void rename(String from, String toDirPath, String toName)
        throws IOException {
        AsyncCommand cmd = sendCommand("renm",
    ...
    public SlaveStatus getSlaveStatus() {
        ...
        try {
            return new SlaveStatus(diskfree, 
            //_roots.getTotalDiskSpaceAvailable(),
            disktotal, //_roots.getTotalDiskSpaceCapacity(),
                bytesSent, bytesReceived, throughputUp, transfersUp,
                throughputDown, transfersDown);
        } catch (Exception ex) {
            ex.printStackTrace();
