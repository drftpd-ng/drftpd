...
public class NukeData {
	 ...
	 * Main constructor.
	 * @param user, Username.
	 * @param path, Path of the dir TO BE nuked.
	 * @param reason, Reason of the nuke/unnuke.
	 * @param nukees, Map of the nuked users.
	 * @param multiplier
	 * @param amount, The total amount of nuked bytes.
	 * @param size, Size of the nuked dir.
	 * @param time, Date of the nuke.
	 */
	public NukeData(String user, String path, String reason, 
			Map<String, Long> nukees, int multiplier, long amount, long size, long time) {
		_user = user;
		_path = path;
		_reason = reason;
		_nukees = nukees;
	...
	/**
	 * This constructor does not need the TIME variable.
	 * Simply add the current time and try to construct the object with the <pre>Main Constructor</pre>.
	 * @param user
	 * @param path
	 * @param reason
	 * @param nukees
	...
	public NukeData(String user, String path, String reason, 
		...
		this(user, path, reason, nukees, multiplier, amount, size, System.currentTimeMillis());
	}
    ...
    /**
     * [NUKE] - Path: /APPS/MICROSOFT.SUCKS - Multiplier: 3 - UsersNuked: 10 - Size: 700MB - Reason: no.thx.
     */
    public String toString() {
    	return "[NUKE] - Path: " + getPath() + " - Multiplier: " + getMultiplier() +
    	" - UsersNuked: " + getNukees().size() + " - Size: " + getSize() +
    	" - Reason: " + getReason();
    }
