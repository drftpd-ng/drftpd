...
public class VirtualDirectory {
	 ...
	 * @deprecated
	 */
	private static void printLine(RemoteFileInterface fl, CharsetEncoder encoder, SocketChannel channel) throws CharacterCodingException, IOException {
		...
		channel.write(encoder.encode(CharBuffer.wrap(line.toString())));
	 ...
	 * @deprecated
	 */
	public static void printList(ArrayList listFiles, SocketChannel channel) throws IOException {
		//out = new BufferedWriter(out);
		//os.write("total 0" + NEWLINE);
		assert channel != null;
		CharsetEncoder encoder = Charset.forName("ISO-8859-1").newEncoder();
		channel.write(encoder.encode(CharBuffer.wrap("total 0"+NEWLINE)));

		// print file list
		for (Iterator iter = listFiles.iterator(); iter.hasNext();) {
			RemoteFileInterface file = (RemoteFileInterface) iter.next();
			printLine(file, encoder, channel);
		}
	}
	 ...
	 */
	public static void printList(Collection files, Writer os)
		throws IOException {
		//out = new BufferedWriter(out);
		os.write("total 0" + NEWLINE);
