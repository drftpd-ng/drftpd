...
public class UserManagment implements CommandHandler {
	...
	private FtpReply doSITE_ADDIP(BaseFtpConnection conn) {
		...
		String args[] = request.getArgument().split(" ");
		if (args.length < 2) {
			return new FtpReply(200, "USAGE: SITE ADDIP <username> <ident@ip>");
		}
		FtpReply response = new FtpReply(200);
		...
		try {
			...
			return new FtpReply(200, "IO Error: " + ex.getMessage());
		}
		return response;
	}
	...
	*NOTE* If you have flag 1 then you DO NOT WANT flag 2
		...
		User myUser;
		{
			...
			if (conn.getUserNull().isGroupAdmin()) {
				...
				if (ratio == 0F) {
					...
					return new FtpReply(200, "ratio can only be 0 or 3");
				}
	 ...
	 */
	private FtpReply doSITE_GADDUSER(BaseFtpConnection conn) {
		return doSITE_ADDUSER(conn);
	}
