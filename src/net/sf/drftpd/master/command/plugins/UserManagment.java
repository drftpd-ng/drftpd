...
public class UserManagment implements CommandHandler, CommandHandlerFactory {
	...
	private FtpReply doSITE_ADDIP(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "addip.usage"));
		}
		String args[] = request.getArgument().split(" ");
		if (args.length < 2) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "addip.specify"));
		}
			...
			for (int i = 1; i < args.length; i++) {
				...
				try {
					...
					response.addComment(conn.jprintf(UserManagment.class
							.getName(), "addip.success", env));
					logger.info("'" + conn.getUserNull().getUsername()
							+ "' added ip '" + string + "' to '"
							+ myUser.getUsername() + "'");
				} catch (DuplicateElementException e) {
					response.addComment(conn.jprintf(UserManagment.class
							.getName(), "addip.dupe", env));
				}
			}
	...
	private FtpReply doSITE_ADDUSER(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			...
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), key));
		}
		...
		if (conn.getUserNull().isGroupAdmin()) {
			...
			try {
				...
				if (users >= conn.getUserNull().getGroupSlots()) {
					return new FtpReply(200, conn.jprintf(UserManagment.class
							.getName(), "adduser.noslots"));
				}
		...
		try {
			...
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "adduser.missingpass"));
		} catch (UserFileException ex) {
		...
		if ("ratio".equals(command)) {
			...
			if (conn.getUserNull().isGroupAdmin()
				...
				if (ratio == 0F) {
					...
					try {
						...
						return new FtpReply(200, "IO error reading userfiles: "
								+ e1.getMessage());
					}
					if (usedleechslots >= conn.getUserNull()
							.getGroupLeechSlots()) {
						return new FtpReply(200, conn.jprintf(
								UserManagment.class.getName(),
								"changeratio.nomoreslots"));
					}
				} else if (ratio != 0F) {
					return new FtpReply(200, conn.jprintf(UserManagment.class
							.getName(), "changeratio.invalidratio"));
				}
				logger.info("'" + conn.getUserNull().getUsername()
						+ "' changed ratio for '" + userToChange.getUsername()
						+ "' from '" + userToChange.getRatio() + "' to '"
						+ ratio + "'");
			...
			try {
				...
				logger.info("'" + conn.getUserNull().getUsername()
						...
						+ userToChange.getUsername() + "' from '"
						+ userToChange.getMaxLogins() + "' '"
						+ userToChange.getMaxLoginsPerIP() + "' to '"
						+ numLogins + "' '" + numLoginsIP + "'");
				userToChange.setMaxLogins(numLogins);
				userToChange.setMaxLoginsPerIP(numLoginsIP);
				env
						.add("numlogins", Long.toString(userToChange
								.getMaxLogins()));
				env.add("numloginsip", Long.toString(userToChange
						.getMaxLoginsPerIP()));
				response.addComment(conn.jprintf(UserManagment.class.getName(),
			...
			try {
				...
				logger.info("'" + conn.getUserNull().getUsername()
						...
						+ userToChange.getUsername() + "' from '"
						+ userToChange.getGroupSlots() + "' "
						+ userToChange.getGroupLeechSlots() + "' to '"
				...
				env.add("groupleechslots", Long.toString(userToChange
						.getGroupLeechSlots()));
				response.addComment(conn.jprintf(UserManagment.class.getName(),
			...
			if (commandArguments.length == 0) {
				try {
					myDate = new SimpleDateFormat("yyyy-MM-dd").parse(
							commandArguments[0]).getTime();
			...
			logger.info("'" + conn.getUserNull().getUsername()
					...
					+ "' from '" + new Date(userToChange.getCreated())
					+ "' to '" + new Date(myDate) + "'");
			...
			logger.info("'" + conn.getUserNull().getUsername()
					...
					+ userToChange.getUsername() + "' from '"
					+ userToChange.getWeeklyAllotment() + "' to "
					+ weeklyAllotment + "'");
	...
	private FtpReply doSITE_CHGRP(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "chgrp.usage"));
		}
		String args[] = request.getArgument().split("[ ,]");
		if (args.length < 2) {
			return FtpReply.RESPONSE_501_SYNTAX_ERROR;
		...
		try {
			...
			return new FtpReply(200, "IO error reading user: " + e.getMessage());
		}
		FtpReply response = new FtpReply(200);
		for (int i = 1; i < args.length; i++) {
			String string = args[i];
	...
	private FtpReply doSITE_CHPASS(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "chpass.usage"));
		}
		String args[] = request.getArgument().split(" ");
		if (args.length != 2) {
			return FtpReply.RESPONSE_501_SYNTAX_ERROR;
	...
	private FtpReply doSITE_DELIP(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "delip.usage"));
		}
		String args[] = request.getArgument().split(" ");
		if (args.length < 2) {
			return FtpReply.RESPONSE_501_SYNTAX_ERROR;
		...
		for (int i = 1; i < args.length; i++) {
			...
			try {
				...
				logger
						.info("'" + conn.getUserNull().getUsername()
	...
	private FtpReply doSITE_DELUSER(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "deluser.usage"));
		}
		if (!conn.getUserNull().isAdmin() && !conn.getUserNull().isGroupAdmin()) {
			return FtpReply.RESPONSE_530_ACCESS_DENIED;
		}
		...
		logger.info("'" + conn.getUserNull().getUsername() + "' deleted user '"
				+ myUser.getUsername() + "'");
		return FtpReply.RESPONSE_200_COMMAND_OK;
	}
	private FtpReply doSITE_GINFO(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "ginfo.usage"));
		}
		//gadmin
	...
	private FtpReply doSITE_KICK(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "kick.usage"));
		}
	...
	private FtpReply doSITE_PASSWD(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "passwd.usage"));
		}
		logger.info("'" + conn.getUserNull().getUsername()
	...
	private FtpReply doSITE_PURGE(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "purge.usage"));
		}
		String delUsername = request.getArgument();
		User myUser;
		try {
	...
	private FtpReply doSITE_READD(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "readd.usage"));
		}
		User myUser;
		try {
			myUser = conn.getConnectionManager().getUserManager()
	...
	private FtpReply doSITE_RENUSER(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "renuser.usage"));
		}
		String args[] = request.getArgument().split(" ");
		if (args.length != 2) {
			return FtpReply.RESPONSE_501_SYNTAX_ERROR;
	...
	private FtpReply doSITE_SEEN(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "seen.usage"));
		}
		User user;
		try {
		...
		return new FtpReply(200, "User was last seen: "
				+ new Date(user.getLastAccessTime()));
	}
	private FtpReply doSITE_TAGLINE(BaseFtpConnection conn) {
		FtpRequest request = conn.getRequest();
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "tagline.usage"));
		}
		logger.info("'" + conn.getUserNull().getUsername()
				+ "' changed his tagline from '"
				+ conn.getUserNull().getTagline() + "' to '"
				+ request.getArgument() + "'");
	...
	private FtpReply doSITE_TAKE(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "take.usage"));
		}
		StringTokenizer st = new StringTokenizer(request.getArgument());
		if (!st.hasMoreTokens()) {
			return FtpReply.RESPONSE_501_SYNTAX_ERROR;
		...
		try {
			...
			logger.info("'" + conn.getUserNull().getUsername() + "' took "
					...
					+ "') from '" + myUser.getUsername() + "'");
			myUser.updateCredits(-credits);
		} catch (Exception ex) {
			return new FtpReply(200, ex.getMessage());
		}
		return new FtpReply(200, "OK, removed " + credits + "b from "
				+ myUser.getUsername() + ".");
	}
	/**
	...
	private FtpReply doSITE_USER(BaseFtpConnection conn) {
		...
		if (!request.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					UserManagment.class.getName(), "user.usage"));
		}
		FtpReply response = (FtpReply) FtpReply.RESPONSE_200_COMMAND_OK.clone();
		User myUser;
		try {
		...
		env.add("userdownloaded", Bytes
				.formatBytes(myUser.getDownloadedBytes()));
	...
	public FtpReply execute(BaseFtpConnection conn)
		...
		throw UnhandledCommandException.create(UserManagment.class, conn
				.getRequest());
	}
