 ...
 * @author mog
 * @author zubov
 * @version $Id: SlaveManagement.java,v 1.1 2004/07/29 19:43:24 zubov Exp $
 */
public class SlaveManagement implements CommandHandlerFactory, CommandHandler {
		...
		for (Iterator iter = conn.getSlaveManager().getSlaves().iterator();
			...
			if (showPlain) {
				...
				try {
					...
					response.addComment(
						conn.jprintf(SlaveManagement.class, "slaves", env));
				} catch (SlaveUnavailableException e) {
					response.addComment(
						conn.jprintf(
							SlaveManagement.class,
							"slaves.offline",
							env));
				}
			}
	...
	private FtpReply doSITE_SLAVE(BaseFtpConnection conn) {
		...
		if (!ftpRequest.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					SlaveManagement.class.getName(), "slave.usage"));
		}
		...
		try {
			rslave = (RemoteSlave) conn.getConnectionManager().getGlobalContext().getSlaveManager().getSlave(slavename);
		} catch (ObjectNotFoundException e) {
		...
		if (!arguments.hasMoreTokens()) {
			if (rslave.getMasks().size() > 0) {
				...
				for (Iterator iter = rslave.getMasks().iterator(); iter.hasNext();) {
		...
		if (command.equalsIgnoreCase("set")) {
			if (arguments.countTokens() != 2) {
				return new FtpReply(501, conn.jprintf(
						SlaveManagement.class.getName(), "slave.set.usage"));
			}
			...
			if (arguments.countTokens() != 1) {
				return new FtpReply(501, conn.jprintf(
						SlaveManagement.class.getName(), "slave.addmask.usage"));
			}
			...
			return new FtpReply(501, conn.jprintf(
					SlaveManagement.class.getName(), "slave.addmask.failed"));
		} else if (command.equalsIgnoreCase("delmask")) {
			if (arguments.countTokens() != 1) {
				return new FtpReply(501, conn.jprintf(
						SlaveManagement.class.getName(), "slave.delmask.usage"));
			}
			String mask = arguments.nextToken();
			env.add("mask",mask);
	...
	private FtpReply doSITE_DELSLAVE(BaseFtpConnection conn) {
		...
		if (!ftpRequest.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					SlaveManagement.class.getName(), "delslave.usage"));
		}
		String argument = ftpRequest.getArgument();
		StringTokenizer arguments = new StringTokenizer(argument);
		if (!arguments.hasMoreTokens()) {
		...
		env.add("slavename",slavename);
		try {
			conn.getConnectionManager().getGlobalContext().getSlaveManager().getSlave(slavename);
		} catch (ObjectNotFoundException e) {
			response.addComment(conn.jprintf(SlaveManagement.class.getName(),
					"delslave.notfound", env));
			return response;
		}
		conn.getConnectionManager().getGlobalContext().getSlaveManager().delSlave(slavename);
		response.addComment(conn.jprintf(SlaveManagement.class.getName(),
	...
	private FtpReply doSITE_ADDSLAVE(BaseFtpConnection conn) {
		...
		if (!ftpRequest.hasArgument()) {
			return new FtpReply(501, conn.jprintf(
					SlaveManagement.class.getName(), "addslave.usage"));
		}
		String argument = ftpRequest.getArgument();
		StringTokenizer arguments = new StringTokenizer(argument);
		if (!arguments.hasMoreTokens()) {
		...
		try {
			conn.getConnectionManager().getGlobalContext().getSlaveManager().getSlave(slavename);
			return new FtpReply(501, conn.jprintf(
					SlaveManagement.class.getName(), "addslave.exists"));
		} catch (ObjectNotFoundException e) {
		}
		conn.getSlaveManager().addSlave(new RemoteSlave(slavename,conn.getSlaveManager()));
		response.addComment(conn.jprintf(SlaveManagement.class.getName(),
