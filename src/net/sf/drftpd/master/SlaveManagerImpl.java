...
public class SlaveManagerImpl
	...
	public void addSlave(String key, Slave slave, RemoteFile remoteroot)
	 ...
	 */
	/*
	protected RemoteFile mergeRoot(
		RemoteSlave slave,
		RemoteFile srcdir,
		RemoteFile dstdir) {
		if (slave == null || srcdir == null) {
			throw new NullPointerException("slave or srcdir cannot be null");
		}

		if (dstdir == null)
			System.out.println("Ja, dstdir är null");
		...
		while (i.hasNext()) {
			...
			if (srcfile.isDirectory()) {
				...
				dstmap.put(
					srcfile.getName(),
					mergeRoot(slave, srcfile, dstfile));
			}
