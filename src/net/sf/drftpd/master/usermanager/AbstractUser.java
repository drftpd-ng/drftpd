 ...
 * @author mog
 * @version $Id: AbstractUser.java,v 1.54 2004/11/09 14:21:28 mog Exp $
 */
public abstract class AbstractUser extends User {
    ...
    public Map getAllObjects() {
        _data = new HashMap();
        putObject(Nuke.LASTNUKED, lastNuked);
        putObject(Nuke.NUKED, timesNuked);
        putObject(Nuke.NUKEDBYTES, nukedBytes);

        putObject(Request.REQUESTS, requests);
        putObject(Request.REQUESTSFILLED, requestsFilled);

        putObject(UserManagment.COMMENT, comment);
        putObject(UserManagment.CREATED, created);
        putObject(UserManagment.RATIO, new Float(ratio));
        putObject(UserManagment.TAGLINE, tagline);

        putObject(RaceStatistics.RACES, racesParticipated);
        putObject(RaceStatistics.RACESLOST, racesLost);
        putObject(RaceStatistics.RACESWON, racesWon);

        putObject(Statistics.LOGINS, logins);
    ...
    private void putObject(Key k, int v) {
        putObject(k, new Integer(v));
    }

    private void putObject(Key k, long v) {
    ...
    public void setDownloadedTimeMonth(int millis) {
        _downloadedMillisecondsMonth = millis;
    }
    ...
    public void setDownloadedTimeWeek(int millis) {
        _downloadedMillisecondsWeek = millis;
    }
