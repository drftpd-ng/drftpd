...
public class AbstractUserTest extends TestCase {
	...
	private static final Logger logger =
		Logger.getLogger(AbstractUserTest.class);
	/**
			...
			if (eventName2 == null)
				throw new RuntimeException(
					"no eventNames to look for when looking for "
						+ event1.getCommand());
			throw new RuntimeException(
				eventName2 + " was expected but wasn't found in " + events);
		}
		if (eventNames.size() != 0)
			throw new RuntimeException(
				"The events "
					+ eventNames
					+ " were not in the list of expected events ("
					+ events
					+ ")");
	}
	...
	public void testResetWeek() throws UserFileException {
		...
		_user.reset(_cm, _resetCal);
		verifyEvents(
			Arrays.asList(new String[] { "RESETDAY", "RESETWEEK" }),
			_cm.events);
	}
}
