...
public class FtpConnection extends BaseFtpConnection {
	...
	*/
	...
	public void doCWD(FtpRequest request, PrintWriter out) {
		...
		if (!connManager
			.getConfig()
			.hasReadPermission(_user, newCurrentDirectory)) {
			FtpResponse response =
	...
	public void doLIST(FtpRequest request, PrintWriter out) {
		...
		for (Iterator iter = listFiles.iterator(); iter.hasNext();) {
			...
			if (!connManager.getConfig().isVisible(user, element))
				iter.remove();
		}
		FtpResponse response =
	...
	public void doPASS(FtpRequest request, PrintWriter out) {
		...
		if (_user.checkPassword(pass)) {
			...
			authenticated = true;
			out.print(response);
			connManager.dispatchFtpEvent(new UserEvent(_user, "LOGIN"));
		} else {
	...
	public void doPORT(FtpRequest request, PrintWriter out) {
		...
		if (!controlSocket
			...
			response.addComment("  to use your real IP: "+controlSocket.getInetAddress().getHostAddress());
			...
			response.addComment("  http://drftpd.mog.se/ for PRET capable clients");
			out.print(response);
			return;
		}
	...
	public void doRETR(FtpRequest request, PrintWriter out) {
		...
		if (!connManager.getConfig().hasReadPermission(_user, _transferFile)) {
			out.print(FtpResponse.RESPONSE_530_ACCESS_DENIED);
			return;
		}
		...
		try {
			...
			_rslave.handleRemoteException(ex);
			out.print(
				FtpResponse.RESPONSE_426_CONNECTION_CLOSED_TRANSFER_ABORTED);
			return;
		} catch (IOException ex) {
			out.print(
				FtpResponse.RESPONSE_426_CONNECTION_CLOSED_TRANSFER_ABORTED);
	...
	public void doRMD(FtpRequest request, PrintWriter out) {
		...
		if (!connManager.getConfig().hasReadPermission(_user, requestedFile)) {
	...
	public void doRNTO(FtpRequest request, PrintWriter out) {
		...
		if(name == null) name = fromFile.getName();
	...
	*NOTE* If you have flag 1 then you DO NOT WANT flag 2
	...
	public void doSITE_CHANGE(FtpRequest request, PrintWriter out) {
		...
		if (command == null) {
			...
			myUser.setCredits(Long.parseLong(commandArgument));
	...
	public void doSITE_DUPE(FtpRequest request, PrintWriter out) {
		...
		findFile(response, currentDirectory.getRoot(), searchstrings);
		out.print(response);
		return;
	}
	...
	public void doSITE_KICKSLAVE(FtpRequest request, PrintWriter out) {
		...
		try {
			rslave =
				connManager.getSlaveManager().getSlave(request.getArgument());
		} catch (ObjectNotFoundException e) {
	...
	public void doSITE_NUKE(FtpRequest request, PrintWriter out) {
		...
		long nukeDirSize = nukeDir.length();
	...
	public void doSITE_PRE(FtpRequest request, PrintWriter out) {
		...
		try {
			...
			out.print(
				new FtpResponse(
					200,
					"release already exists in target section"));
			return;
		} catch (IllegalTargetException ex) {
			out.print(
				new FtpResponse(200, "Illegal arguments: " + ex.getMessage()));
			logger.log(Level.WARN, "illegal arguments", ex);
		}
	...
	public void doSITE_RELOAD(FtpRequest request, PrintWriter out) {
		...
		try {
			connManager.getConfig().reloadConfig();
		...
		connManager.dispatchFtpEvent(new UserEvent(_user, "RELOAD"));
		out.print(FtpResponse.RESPONSE_200_COMMAND_OK);
		return;
	}
	...
	public void doSITE_RESCAN(FtpRequest request, PrintWriter out) {
		...
		for (Iterator i = sfv.getEntries().entrySet().iterator();
			...
			String status;
			long fileCheckSum = file.getCheckSum(true);
			if (fileCheckSum == 0L) {
	...
	public void doSITE_STATS(FtpRequest request, PrintWriter out) {
		...
		response.addComment(
			"week: "
				...
				+ "b "
				+ Bytes.formatBytes(user.getDownloadedFilesWeek())
				+ "f ");
		response.addComment(
			"day: "
	...
	public void doSITE_WHO(FtpRequest request, PrintWriter out) {
		...
		for (Iterator i = connManager.getConnections().iterator();
			...
			if (connManager
				.getConfig()
				.checkHideInWho(conn.getCurrentDirectory(), _user)) {
				continue;
			}
			if (conn.isExecuting()) {
	...
	public void doSTOR(FtpRequest request, PrintWriter out) {
			...
			out.print(
				new FtpResponse(
					...
					"IO Error from "
						...
						+ ": "
						+ ex.getMessage()));
			logger.log(Level.WARN, "IO Error from " + _rslave.getName(), ex);
			return;
		}
		...
		try {
			...
			response.addComment(
				"zipscript - slave with .sfv file is offline, no checksum matching performed");
		} catch (ObjectNotFoundException e) {
			response.addComment("zipscript - no .sfv file in directory");
		} catch (IOException e) {
			response.addComment("zipscript - IO error: " + e.getMessage());
		}
