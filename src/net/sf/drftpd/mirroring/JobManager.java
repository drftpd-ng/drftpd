...
/**
 * @author matt
 * @version $Id: JobManager.java,v 1.1 2003/12/11 18:19:26 zubov Exp $
 */
public class JobManager implements FtpListener {
	...
	public void processJob(RemoteSlave slave) {
		...
		logger.info("Sending " + temp.getFile().getName() + " to " + slave.getName());
		...
		try {
			...
			logger.error("Error Sending " + temp.getFile().getName() + " to " + slave.getName());
			return;
		}
		logger.info("Done Sending " + temp.getFile().getName() + " to " + slave.getName() + " in " + difference/1000 + " seconds");
				...
				try {
					if ( myMirrorJob == null ) {
						...
						if (tempJob.getFile().getAvailableSlaves() == null ) {
							logger.error("tempJob.getFile().getAvailableSlaves() == null");
							continue;
						}
						if ( !tempJob.getFile().getAvailableSlaves().contains(slave)) {
								logger.info("myMirror is assigned - " + slave.getName());
								myMirrorJob = tempJob;
						}
					}
				} catch (NoAvailableSlaveException e) {
					logger.info("NoAvailableSlaveException for myMirrorJob - " + slave.getName());
					// can't transfer it, so don't set myMirrorJob
				}
				continue;
			}
			try {
				if (tempJob.getDestinationSlaves().contains(slave) && !tempJob.getFile().getAvailableSlaves().contains(slave)) {
					logger.info("tempJob is being returned - " + slave.getName());
					return tempJob;
				}
			} catch (NoAvailableSlaveException e) {
				// continue searching through jobs
				logger.info("NoAvailableSlaveException for tempJob - " + slave.getName());
			}
		}
		if ( myMirrorJob != null ) {
	...
	public void actionPerformed(Event event) {
		...
		if (slaveEvent.getCommand() == "DELSLAVE") {
			...
			JobManagerThread newThread = new JobManagerThread(slaveEvent.getRSlave(), this);
			_threadList.add(newThread);
			newThread.start();
		}
		logger.info(
