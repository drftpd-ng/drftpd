...
/**
 * @author mog
 * @version $Id: PortRangeServerSocketFactory.java,v 1.6 2004/08/03 20:14:03 zubov Exp $
 * @deprecated
 */
public class PortRangeServerSocketFactory extends RMISocketFactory {
    private static final Logger logger = Logger.getLogger(PortRangeServerSocketFactory.class);
    int _portfrom;
    int _portto;

    public PortRangeServerSocketFactory(int portfrom, int portto) {
        if (portfrom > portfrom) {
            throw new IllegalArgumentException("from cannot be higher than to");
        }
    ...
    public ServerSocket createServerSocket(int port) throws IOException {
        if (port != 0) {
            logger.error("Returning anonymous port to non-anonymous request",
                new Throwable());
        }
            ...
            try {
                ...
                if (myport == randport) {
                    throw (IOException) new IOException(
                        "No available ports to listen on").initCause(e);
                }
            }
        }
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof PortRangeServerSocketFactory) {
            ...
            return ((factory._portfrom == _portfrom) &&
            (factory._portto == _portto));
        }
