...
implements Slave, Unreferenced {
    private static final Logger logger = Logger.getLogger(SocketSlaveImpl.class.getName());
    ...
    private class Command {
        ...
        public Hashtable getData()      { return _data; }
        public int getStatus()          { return _stat; }
        public void setStatus(int stat) { _stat = stat; }
    ...
    private void process(Command cmd) {
        if (cmd.getName().equals("ping")) { processPing(cmd); return; }
        if (cmd.getName().equals("send")) { processSend(cmd); return; }
        if (cmd.getName().equals("recv")) { processRecv(cmd); return; }
        if (cmd.getName().equals("dump")) { processDump(cmd); return; }
        if (cmd.getName().equals("csum")) { processCRC32(cmd); return; }
        if (cmd.getName().equals("renm")) { processRename(cmd); return; }
        if (cmd.getName().equals("dele")) { processDelete(cmd); return; }
        if (cmd.getName().equals("conn")) { processConnect(cmd); return; }
        if (cmd.getName().equals("disk")) { processDisk(cmd); return; }
        if (cmd.getName().equals("list")) { processList(cmd); return; }
    }
    ...
    private void processPing(Command cmd) {
        sendLine("ping");
        if (readLine(5) == null) cmd.setStatus(-1);
    }
    ...
    private void processCRC32(Command cmd) {
        ...
        if ((res = readLine(5)) == null) {
            ...
            // parse response
            if (res.startsWith("CRCFAIL ")) { cmd.setStatus(1); return; }
    ...
    private void processDump(Command cmd) {
        ...
        if ((res = readLine(5)) == null) {
            ...
            // parse response
            if (res.startsWith("DUMPFAIL ")) { cmd.setStatus(1); return; }
            StringBuffer buf = new StringBuffer(65536);
            while ((res = readLine(5)) != null) {
                if (res.equals("DUMPEND")) break;
                buf.append(res);
            }
            if (res == null) cmd.setStatus(1);
    ...
    private void processDelete(Command cmd) {
        ...
        if ((res = readLine(5)) == null) {
            ...
            // parse response
            if (res.startsWith("DELFAIL ")) cmd.setStatus(1);
        }
    }
    ...
    private void processRename(Command cmd) {
        String msg = "ren " + 
            ...
            "\"" + cmd.getArgs().get("path") + "/" +cmd. getArgs().get("name") + "\"";
        sendLine(msg);
        String res;
        if ((res = readLine(5)) == null) {
            cmd.setStatus(-1);
            ...
            // parse response
            if (res.startsWith("RENFAIL ")) cmd.setStatus(1);
        }
    }
    ...
    private void processList(Command cmd) {
        ...
        if ((txt = readLine(20)) == null || txt.startsWith("LISTFAIL")) {
            ...
            while ((txt = readLine(20)) != null) {
                if (txt.equals("LISTEND")) break;
                if (txt.equals("LISTBEGIN")) continue;
                if (!txt.startsWith("/") && !txt.equals("") && txt.indexOf("type=dir;")==-1)
                    sbuf.append("x.slaves=" + _name + ";");
                sbuf.append(txt);
                sbuf.append((String)"\n");
		}
		try {
                LinkedRemoteFile root = MLSTSerialize.unserialize(
                    _cman.getConfig(),
    ...
    public Hashtable doCommand(
    String name, Hashtable args
    ) {
        ...
        if (cmd.getStatus() == -1) return null;
        return cmd.getData();
    }
    ...
    public long checkSum(String path) throws IOException {
        ...
        if (result == null) throw new IOException("Checksum command failed");
    ...
    public String dumpfile(String path) throws IOException {
        ...
        if (result == null) throw new IOException("Dump command failed");
    ...
    public void delete(String path) throws IOException {
        ...
        if (result == null) throw new IOException("Delete command failed");
        return;
    }
        ...
        throws IOException 
    {
        ...
        if (result == null) throw new IOException("Rename command failed");
        return;
    }
    ...
    public SlaveStatus getSlaveStatus() {
        ...
        try {
            ...
            diskfree, //_roots.getTotalDiskSpaceAvailable(),
