...
/**
 * @author mog
 * @version $Id: JDOMSerialize.java,v 1.11 2004/08/03 20:14:02 zubov Exp $
 */
public class JDOMSerialize {
    private static final Logger logger = Logger.getLogger(JDOMSerialize.class);

    public static LinkedRemoteFileInterface unserialize(ConnectionManager cm,
        Reader in, List rslaves) throws FileNotFoundException {
        LinkedRemoteFileInterface root;

        try {
            Document doc = new SAXBuilder().build(in);
            JDOMRemoteFile xmlroot = new JDOMRemoteFile(doc.getRootElement(),
                    rslaves);
            root = new LinkedRemoteFile(xmlroot,
