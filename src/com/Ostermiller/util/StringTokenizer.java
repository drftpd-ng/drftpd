 ...
 * href= "http://ostermiller.org/utils/StringTokenizer.html">ostermiller.org</a>.
 * <p>
 * The tokenization method is much simpler than the one used by the
 * <code>StreamTokenizer</code> class. The <code>StringTokenizer</code>
 ...
 * <p>
 * In the original version of <code>java.util.StringTokenizer</code>, the
 * method <code>nextToken()</code> left the current position after the
 * returned token, and the method <code>hasMoreTokens()</code> moved (as a
 * side effect) the current position before the beginning of the next token.
 * Thus, the code: <blockquote><code>
 * String s = "x=a,b,c";<br>
 ...
 * <code>hasMoreTokens</code> method: now, it does not advance current
 * position. However, after these changes the output of the above code was:
 * <blockquote> x<br>
 * =a<br>
 * b<br>
 * c </blockquote>
 ...
 * <code>skipDelimiters()</code>. To produce the original output, the above
 * code should be modified as: <blockquote><code>
 * String s = "x=a,b,c";<br>
...
public class StringTokenizer implements java.util.Enumeration,
	 ...
	 *            delimiters is empty; <code>true</code> -- the second
	 *            parameter specifies token delimiters, the set of nontoken
	 *            delimiters is empty.
	 * @throws NullPointerException
	 *             if text is null.
	 * 
	...
	public StringTokenizer(String text, String delims, boolean delimsAreTokens) {
		this(text, (delimsAreTokens ? null : delims), (delimsAreTokens ? delims
				: null));
	}
	...
	private void setDelims(String nontokenDelims, String tokenDelims) {
		...
		for (int i = 0; (nontokenDelims != null)
				&& (i < nontokenDelims.length()); i++) {
	...
	public String nextToken() {
		...
		while ((position != workingPosition)
			...
			if (onToken) {
				...
				return (emptyReturned ? "" : text.substring(workingPosition,
	...
	public void setDelimiters(String delims, boolean delimsAreTokens) {
		setDelims((delimsAreTokens ? null : delims), (delimsAreTokens ? delims
				: null));
	}
	...
	private boolean advancePosition() {
		...
		if (returnEmptyTokens
			...
			if (((c <= maxDelimChar) && ((nontokenDelims != null) && (nontokenDelims
					.indexOf(c) != -1)))
		...
		while (position != -1) {
			...
			if (returnEmptyTokens && !emptyReturned
				...
				if ((c <= maxDelimChar)
						&& (c1 <= maxDelimChar)
						&& (((nontokenDelims != null) && (nontokenDelims
								.indexOf(c) != -1)) || ((tokenDelims != null) && (tokenDelims
								.indexOf(c) != -1)))
						&& (((nontokenDelims != null) && (nontokenDelims
								.indexOf(c1) != -1)) || ((tokenDelims != null) && (tokenDelims
								.indexOf(c1) != -1)))) {
					emptyReturned = true;
			...
			int nextDelimiter = ((position < (strLength - 1)) ? indexOfNextDelimiter(position + 1)
					: (-1));

			if ((c > maxDelimChar)
					|| (((nontokenDelims == null) || (nontokenDelims.indexOf(c) == -1)) && ((tokenDelims == null) || (tokenDelims
							.indexOf(c) == -1)))) {
				// token found
				 ...
				 * ".");
				 */
				position = ((position < (strLength - 1)) ? (position + 1)
						: (-1));
	 ...
	 * <code>tokenDelims</code> and <code>nontokenDelims</code>,
	 * respectively. Then the next token (with respect to new delimiters) in the
	 * string after the current position is returned.
	 * <p>
	 * The current position is set after the token returned.
	 * <p>
	 ...
	 * First, the sets of token and nontoken delimiters are changed to be the
	 * <code>tokenDelims</code> and <code>nontokenDelims</code>,
	 * respectively; and whether or not to return empty tokens is set. Then the
	 * next token (with respect to new delimiters) in the string after the
	 ...
	 * @param delimsAreTokens
	 *            flag indicating whether the first parameter specifies token or
	 *            nontoken delimiters: <code>false</code> -- the first
	 *            parameter specifies nontoken delimiters, the set of token
	 *            delimiters is empty; <code>true</code> -- the first
	 *            parameter specifies token delimiters, the set of nontoken
	 *            delimiters is empty.
	 * @return the next token, after switching to the new delimiter set.
	 * @throws NoSuchElementException
	...
	public String nextToken(String delims, boolean delimsAreTokens) {
		return (delimsAreTokens ? nextToken(null, delims) : nextToken(delims,
				null));
	}
	 ...
	 * <code>UnsupportedOperationException</code>. It exists so that this
	 * class can implement the <code>Iterator</code> interface.
	 * 
	 ...
	 * state of the tokenizer such that <code>hasMoreTokens()</code> will
	 * return false.
	 * 
